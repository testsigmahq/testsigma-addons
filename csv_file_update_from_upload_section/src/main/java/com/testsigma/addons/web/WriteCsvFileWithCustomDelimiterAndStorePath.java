...
@Data
@Action(actionText = "Write row number and column number into CSV file with testdata where absolutepath is test_data with delimiter delimiter-value and store filepath in runtime variable variable-name (It supports file from upload section)", description = "Write a particular row and column into a delimiter-separated file (e.g. pipe | for pipe-separated files, comma , for standard CSV). Can accept local file paths or URLs. Stores the file path in a runtime variable. It supports file from upload section.", applicationType = ApplicationType.WEB)
public class WriteCsvFileWithCustomDelimiterAndStorePath extends WebAction {
