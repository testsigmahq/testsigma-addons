...
@Data
@Action(actionText = "Clear cell value from a CSV file file-path where row is row-number and column is column-number with delimiter delimiter-value",
        description = "Deletes a particular cell from a delimiter-separated file (e.g. pipe | for pipe-separated files, comma , for standard CSV) using 1-based indexing for row and column numbers.",
        applicationType = ApplicationType.WEB)
public class DeleteRowContentFromCsvWithCustomDelimiter extends WebAction {
        ...
        try (Reader reader = new FileReader(csvFilePath);
            ...
            if (rowIndex >= 0 && rowIndex < rows.size()) {
                ...
                if (columnIndex >= 0 && columnIndex < row.length) {
                    ...
                    setSuccessMessage("Content deleted from row " + targetRow + " and column " + targetColumn);
                } else {
