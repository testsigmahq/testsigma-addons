...
@Action(
        ...
        description = "Dynamically calls a gRPC service using a .proto file and JSON request. Automatically downloads standard proto imports, generates descriptors at runtime, and stores the JSON response in a runtime variable.",
...
public class GrpcClientAction extends WebAction {
    ...
    public Result execute() {
        ...
        Path tempImportsDir = null;

        try {
            tempImportsDir = Files.createTempDirectory("proto-imports");
            GrpcClientUtils.downloadProtoImports(tempImportsDir, logger);
            ...
            tempDescriptorFile = GrpcClientUtils.generateDescriptor(protoDir, protoFileName, tempImportsDir, logger);
            ...
            if (tempImportsDir != null) {
                try {
                    GrpcClientUtils.deleteDirectoryRecursively(tempImportsDir);
                } catch (Exception e) {
                    logger.warn("Failed to delete temporary imports directory: " + tempImportsDir + e);
                }
            }
        }
    }
