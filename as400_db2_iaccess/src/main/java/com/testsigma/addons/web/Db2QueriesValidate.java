...
@Data
@Action(actionText = "Execute DB2 Query on the connection with host host, port port, database dbname, username username, password password and verify affected rows count is Row-Count",
description = "This Action executes given DB2 SQL query and validates the affected rows.",
applicationType = ApplicationType.WEB)
public class Db2QueriesValidate extends WebAction {
	...
	public Result execute() throws NoSuchElementException {
		...
		try{
			...
			if(query.trim().toUpperCase().startsWith("SELECT")) {
				...
				sb.append("Successfully Executed DB2 Query and Rows fetched from DB : " + rowsUpdatedOrFetched + "<br>");
			}else {
				rowsUpdatedOrFetched = stmt.executeUpdate(query);
				sb.append("Successfully Executed DB2 Query, No. of rows affected in DB : " + rowsUpdatedOrFetched + "<br>");
			}
			...
			if(rowsUpdatedOrFetched == Integer.parseInt(testData7.getValue().toString())) {
				...
				sb.append("Expected no. of affected rows:" + testData7.getValue().toString() + "<br>");
