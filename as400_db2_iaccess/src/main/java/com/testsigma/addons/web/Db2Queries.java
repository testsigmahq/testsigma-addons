...
public class Db2Queries extends WebAction {
	...
	public Result execute() throws NoSuchElementException {
		...
		if(query.trim().toUpperCase().startsWith("SELECT")) {
			...
			sb.append("<br>Total rows fetched: " + rowCount + "<br>");
		} else {
			// Handle non-SELECT queries (INSERT, UPDATE, DELETE, etc.)
			int rowsAffected = stmt.executeUpdate(query);
			sb.append("Successfully Executed DB2 Query: " + query.trim().toUpperCase().split("\\s+")[0] + "<br>");
			sb.append("Number of rows affected: " + rowsAffected + "<br>");
		}
