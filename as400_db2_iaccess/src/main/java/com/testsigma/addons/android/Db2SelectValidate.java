...
public class Db2SelectValidate extends AndroidAction {
		...
		try{
			...
			if (resultSet.next()) {
				...
				if(testData7.getValue().toString().equals(resultData)) {
					sb.append("\nThe output from the DB2 Select Query is matching with expected value.");
					sb.append("\nExpected value:" + testData7.getValue().toString());
					sb.append("\nActual output from query:" + resultData);
					setSuccessMessage(sb.toString());
					logger.info(sb.toString());
				}
				else {
					result = Result.FAILED;
					sb.append("The selected query value not match with expected value:" + "\n");
					sb.append("Expected value of select query:" + testData7.getValue().toString() + "\n");
