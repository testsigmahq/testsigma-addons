...
@Data
public class EnterInTextAreaBasedOnAttributeValues extends WebAction {

    private static final String NOT_UNIQUE_ERROR = "There is more than one occurrence for the textarea with %s%s \"%s\"";
    private static final String SUCCESS_MESSAGE = "Successfully entered data \"%s\" into the textarea with %s%s \"%s\"";
    private static final String FAILURE_MESSAGE_NODATA = "Found element, But unable to enter given test data <b>\"%s\"</b>. <br>" +
            "Please verify if the textarea with %s %s <b>%s</b> is enabled and accepting data.";
    private static final String ELEMENT_NOT_FOUND = "Unable to find a textarea with %s%s <b>\"%s\"</b>." +
            "<br>If the textarea is present in the page, please verify the %s of textarea%s given value";
        ...
        if (isLabelText.equals(true)) {
            xpath = operator.equals("equals") ?
                    (isContainerLabel != null ?
                            "//label[text()='" + value.getValue() + "']//following::textarea[1]"
                            : (isFollowingText != null && isFollowingText.equals(true) ?
                            "//label[text()='" + value.getValue() + "']//preceding::textarea[1]" :
                            "//label[text()='" + value.getValue() + "']//following::textarea[1]"))
                    : (isContainerLabel != null ?
                    "//label//text()[contains(.,'" + value.getValue() + "')]//following::textarea[1]"
                    : (isFollowingText != null && isFollowingText.equals(true) ?
                    "//label[contains(text(),'" + value.getValue() + "')]//preceding::textarea[1]" :
                    "//label[contains(text(),'" + value.getValue() + "')]//following::textarea[1]"));
        } else {
            if (attribute.equals("text")) {
                xpath = operator.equals("equals") ?
                        (isFollowingText != null && isFollowingText.equals(true) ?
                                "//*[text()='" + value.getValue() + "']//preceding::textarea[1]" :
                                "//*[text()='" + value.getValue() + "']//following::textarea[1]")
                        :
                        (isFollowingText != null && isFollowingText.equals(true) ?
                                "//*[contains(text(),'" + value.getValue() + "')]//preceding::textarea[1]" :
                                "//*[contains(text(),'" + value.getValue() + "')]//following::textarea[1]");
            } else {
                xpath = operator.equals("equals") ?
                        "//textarea[@" + attribute + "" + value.getValue() + "']" :
                        "//textarea[contains(@" + attribute + ",'" + value.getValue() + "')]";
            }
        }
