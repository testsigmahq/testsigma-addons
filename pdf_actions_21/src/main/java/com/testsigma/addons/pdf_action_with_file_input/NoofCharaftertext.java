...
@Data
@Action(actionText = "Extract No_Of_Character characters after occurrence Occurrence of text test_data from PDF at file path pdf_file_path and store in a runtime variable Variable_Name",
        description = "Extracts the characters from the PDF (file path or URL) after the specified occurrence of test_data (supports literal or regex). Use Occurrence 1 for first, 2 for second, etc. Use_Regex true for regex pattern.",
        applicationType = ApplicationType.WEB)
public class NoofCharaftertext extends PdfFileInputBase {
    ...
    public Result execute() {
        ...
        try (PDDocument doc = loadPdfDocument(basePDF)) {
            ...
            String extracted = PdfExtractionUtil.extractCharsAfter(content, match, noofchar);

            runTimeData = new com.testsigma.sdk.RunTimeData();
            runTimeData.setValue(extracted);
            runTimeData.setKey(testData3.getValue().toString());
            setSuccessMessage("Extracted " + testData1.getValue() + " characters i.e " + extracted + " and stored it in runtime variable " + testData3.getValue().toString());
