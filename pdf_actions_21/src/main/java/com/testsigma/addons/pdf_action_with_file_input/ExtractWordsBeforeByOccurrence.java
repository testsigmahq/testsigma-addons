...
@Data
@Action(actionText = "Extract No_Of_Words words before occurrence Occurrence of search_pattern from PDF at file path pdf_file_path using match type Match_Type and store in a runtime variable Variable_Name",
        description = "Extracts N words before the specified occurrence of search_pattern. Match_Type: choose 'Text' for literal text match or 'Regex' for regex pattern. Occurrence: 1=first, 2=second, etc.",
        applicationType = ApplicationType.WEB)
public class ExtractWordsBeforeByOccurrence extends PdfFileInputBase {
    ...
    public Result execute() {
        ...
        try (PDDocument doc = loadPdfDocument(basePDF)) {
            ...
            String extracted = PdfExtractionUtil.extractWordsBefore(content, match, numWords);

            runTimeData = new com.testsigma.sdk.RunTimeData();
            runTimeData.setValue(extracted);
            runTimeData.setKey(variableName.getValue().toString());
            setSuccessMessage("Extracted " + numWords + " words before occurrence " + occurrenceNum + " i.e " + extracted + " and stored in runtime variable " + variableName.getValue());
