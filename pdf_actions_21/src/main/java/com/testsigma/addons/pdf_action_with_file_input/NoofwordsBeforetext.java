...
@Data
@Action(actionText = "Extract No_Of_Words words before occurrence Occurrence of text test_data from PDF at file path pdf_file_path and store in a runtime variable Variable_Name",
        description = "Extracts the words from the PDF (file path or URL) before the specified occurrence of test_data (supports literal or regex). Use Occurrence 1 for first, 2 for second, etc. Use_Regex true for regex pattern.",
        applicationType = ApplicationType.WEB)
public class NoofwordsBeforetext extends PdfFileInputBase {
    ...
    public Result execute() {
        ...
        try (PDDocument doc = loadPdfDocument(basePDF)) {
            ...
            String extracted = PdfExtractionUtil.extractWordsBefore(content, match, wordcount);

            runTimeData = new com.testsigma.sdk.RunTimeData();
            runTimeData.setValue(extracted);
            runTimeData.setKey(testData3.getValue().toString());
            setSuccessMessage(SUCCESS_MESSAGE + " Previous " + testData1.getValue() + " words are " + extracted + " stored in runtime variable " + testData3.getValue().toString());
