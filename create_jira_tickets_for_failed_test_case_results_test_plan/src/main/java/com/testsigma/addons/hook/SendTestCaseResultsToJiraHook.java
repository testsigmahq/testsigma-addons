    ...
    protected Result execute() {
        ...
        try {
            ...
            if (errors > 0 && ticketsCreated == 0) {
                ...
                setErrorMessage("Jira ticket creation failed for all " + failedTestCases.size()
                        + " failed test case(s). Errors: " + errors + "." + errDetail);
                return Result.FAILED;
            }
            if (errors > 0 && ticketsCreated > 0) {
                String errDetail = errorMessages.isEmpty() ? "" : " First error: " + errorMessages.get(0);
            ...
            if (ticketsCreated == 0) {
                setErrorMessage("No Jira tickets were created for " + failedTestCases.size()
                        + " failed test case(s). All were skipped as duplicates. Verify Jira project key, issue type, and that no open ticket with the same summary already exists.");
                return Result.FAILED;
            }
    ...
    private JsonArray fetchFailedTestCases(Long runId, String apiToken) throws Exception {
        ...
        while (hasMore) {
            ...
            if (response.statusCode() != 200) {
                ...
                throw new Exception("Testsigma API error: failed to fetch test case results. Status "
