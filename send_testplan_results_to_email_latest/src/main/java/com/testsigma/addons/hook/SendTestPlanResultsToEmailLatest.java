...
@Data
@TestPlanHook(name = "Send Test Plan Results to Email Latest", type = HookType.AFTER)
public class SendTestPlanResultsToEmailLatest extends Hook {
    ...
    private static final String TEST_CASE_RESULTS_QUERY_BASE = "iteration:null,executionResultId:%d,isStepGroup:false,afterTestParentResultId:null,dependentPrerequisiteCaseResultId:null";
...
//        Long runId = runresult.getId();
        ...
        if (toList.isEmpty()) {
            setErrorMessage(
                    "TO EMAIL is required. Please set the TO EMAIL test data (comma-separated for multiple recipients).");
            return Result.FAILED;
        }
        if (host.isEmpty()) {
        ...
        String from = (DEFAULT_FROM_EMAIL != null && !DEFAULT_FROM_EMAIL.isEmpty()) ? DEFAULT_FROM_EMAIL.trim()
                : toList.get(0);
        ...
        try {
            ...
            TestCaseResultsData resultsData = fetchTestCaseResultsData(runId, runIdForFirstRunCheck, tsApiToken,
                    httpClient);

            boolean isFirstRun = isFirstRunFromRd != null
                    ? isFirstRunFromRd
                    : (resultsData.isFirstRun != null ? resultsData.isFirstRun
                            : isFirstRunFromTestCaseResults(runIdForFirstRunCheck, tsApiToken, httpClient));

            // Prefer counts from execution_result (current run details) to avoid extra API
            // call
            ...
            if (counts == null && resultsData.planCountsFromApi != null)
                counts = resultsData.planCountsFromApi;
            if (counts == null)
    ...
    private List<JsonObject> fetchRunDetailsChain(List<Long> runIdsInOrder, String apiToken, HttpClient httpClient)
            throws Exception {
    ...
    /**
     * Fetches failed test cases and optionally isFirstRun from first page when
     * runId == runIdForFirstRunCheck.
     * Counts are not computed here; use getCountsFromRunDetails
     * (testPlanResultMetric) instead.
     */
    private TestCaseResultsData fetchTestCaseResultsData(Long runId, Long runIdForFirstRunCheck, String apiToken,
        ...
        while (hasMore) {
            ...
            if (response.statusCode() != 200) {
                ...
                throw new Exception("Testsigma API error. Status " + response.statusCode() + ". "
                        + (apiMsg != null ? apiMsg : body));
            }
        ...
        for (int i = 0; i < content.size(); i++) {
            ...
            if (item.has("environmentResult") && item.get("environmentResult").isJsonObject()) {
                ...
                if (envResult.has("executionResult") && envResult.get("executionResult").isJsonObject()) {
                    ...
                    if (execResult.has("lastReRunResult")
                            && isMeaningfulLastReRunResult(execResult.get("lastReRunResult")))
                        return false;
                }
            }
        }
    ...
    private static class TestCaseResultsData {
        ...
        /**
         * Counts from content[].environmentResult.executionResult.testPlanResultMetric
         * when present.
         */
    ...
    private Map<String, Integer> getPlanCountsFromTestCaseResultContent(JsonObject contentItem) {
        if (contentItem == null)
            return null;
        if (!contentItem.has("environmentResult") || !contentItem.get("environmentResult").isJsonObject())
            return null;
        JsonObject envResult = contentItem.getAsJsonObject("environmentResult");
        if (!envResult.has("executionResult") || !envResult.get("executionResult").isJsonObject())
            return null;
        JsonObject execResult = envResult.getAsJsonObject("executionResult");
        if (!execResult.has("testPlanResultMetric") || !execResult.get("testPlanResultMetric").isJsonObject())
            return null;
    ...
    /**
     * Builds counts map from testPlanResultMetric using only consolidated plan
     * fields.
     */
    ...
    private JsonObject fetchExecutionResult(Long runId, String apiToken, HttpClient httpClient) throws Exception {
        ...
        if (response.statusCode() != 200) {
            ...
            throw new Exception("Failed to fetch execution result. Status " + response.statusCode() + ". "
                    + (msg != null ? msg : body));
        }
    ...
    /**
     * True if lastReRunResult is a non-null object with id or executionResultId
     * (indicates a rerun exists).
     */
    ...
    /**
     * Used only when runIdForFirstRunCheck != current runId (e.g. chain: need root
     * run's first page).
     */
    ...
    /**
     * Fetches test_case_results with executionResultId = lastRunId (one page),
     * extracts Total, Passed, Failure, etc.
     * from content[].environmentResult.executionResult.testPlanResultMetric
     * (totalCount, passedCount, failedCount, ...).
     */
    private Map<String, Integer> fetchPlanCountsFromTestCaseResultsByExecutionResultId(Long executionResultId,
            String apiToken, HttpClient httpClient) {
        try {
            ...
            log("Failed to fetch plan counts from test_case_results for executionResultId " + executionResultId + ": "
                    + e.getMessage());
        }
        return null;
    }
    ...
    private static final DateTimeFormatter TIMESTAMP_FORMAT = DateTimeFormatter
            .ofPattern("yyyy-MM-dd HH:mm:ss", Locale.US).withZone(ZoneOffset.UTC);

    /**
     * Format timestamp from API: epoch ms (or seconds if value < 1e12) ->
     * "yyyy-MM-dd HH:mm:ss UTC".
     */
    private String formatTimestamp(JsonObject rd, String key, String fallbackKey) {
        JsonElement el = null;
        if (rd.has(key) && !rd.get(key).isJsonNull())
            el = rd.get(key);
        else if (fallbackKey != null && rd.has(fallbackKey) && !rd.get(fallbackKey).isJsonNull())
            el = rd.get(fallbackKey);
        if (el == null)
            return "N/A";
    ...
    /**
     * Reads consolidated counts from testPlanResultMetric in run details
     * (execution_results API).
    ...
    private int getInt(JsonObject obj, String primaryKey, String fallbackKey, int defaultValue) {
        ...
        if (fallbackKey != null && obj.has(fallbackKey) && !obj.get(fallbackKey).isJsonNull())
            return obj.get(fallbackKey).getAsInt();
        return defaultValue;
    }

    /**
     * First run from run details: single run with no reRunParentId. Returns null if
     * unknown.
     */
    ...
    private String getTestPlanNameFromRunDetails(JsonObject runDetails) {
        ...
        if (obj.has("execution") && obj.get("execution").isJsonObject()) {
            ...
            if (execution.has("name") && !execution.get("name").isJsonNull())
                return execution.get("name").getAsString();
        }
        if (obj.has("name") && !obj.get("name").isJsonNull())
            return obj.get("name").getAsString();
        if (obj.has("testPlanName") && !obj.get("testPlanName").isJsonNull())
            return obj.get("testPlanName").getAsString();
        if (obj.has("testPlan") && obj.get("testPlan").isJsonObject()) {
            JsonObject tp = obj.getAsJsonObject("testPlan");
            if (tp.has("name"))
                return tp.get("name").getAsString();
        }
        return "Test Plan";
    }

    /**
     * lastRunId from run details (execution.lastRunId or rd.lastRunId). For rerun,
     * use this as the execution link RunId.
     */
    private Long getLastRunIdFromRunDetails(JsonObject runDetailsRaw) {
        ...
        if (rd.has("execution") && rd.get("execution").isJsonObject()) {
            JsonObject exec = rd.getAsJsonObject("execution");
            if (exec.has("lastRunId") && !exec.get("lastRunId").isJsonNull())
                return exec.get("lastRunId").getAsLong();
        }
        if (rd.has("lastRunId") && !rd.get("lastRunId").isJsonNull())
            return rd.get("lastRunId").getAsLong();
        return null;
    }

    private String buildEmailHtml(Long runId, List<JsonObject> allRunDetails, Map<String, Integer> counts,
            JsonArray failedTestCases, boolean isFirstRun) {
        ...
        // Resolve test plan name: try current run (last) first, then first, then any
        // run that has execution.name
        ...
        if (firstRun != null && firstRun.has("execution") && firstRun.get("execution").isJsonObject()) {
            JsonObject execution = firstRun.getAsJsonObject("execution");
            if (execution.has("id") && !execution.get("id").isJsonNull())
                executionResultId = execution.get("id").getAsLong();
        } else if (firstRun != null && firstRun.has("id") && !firstRun.get("id").isJsonNull()) {
            executionResultId = firstRun.get("id").getAsLong();
        }
        String xlsReportUrl = TESTSIGMA_APP_BASE_URL + "/ui/td/execution_results/generate_xls/" + executionResultId
                + "/runs/" + runId;

        StringBuilder sb = new StringBuilder();
        sb.append(
                "<!DOCTYPE html><html><head><meta charset=\"UTF-8\"></head><body style=\"font-family: Arial, sans-serif; font-size: 14px; color: #333; line-height: 1.5;\">");
        sb.append("<p>Hi,</p>");
        ...
        // Run Result and Re-Run #1, Re-Run #2, ... (indentation: heading left; Start
        // Time, End Time indented one level)
        if (allRunDetails != null && !allRunDetails.isEmpty()) {
            for (int i = 0; i < allRunDetails.size(); i++) {
                if (i > 0)
                    sb.append("<p style=\"margin-top: 0.5em;\"></p>"); // blank line between sections
                ...
                sb.append(
                        "<p style=\"margin-left: 1.5em; margin-top: 0.25em; margin-bottom: 0.25em;\"><strong>Status:</strong> ")
                        .append("Completed").append("</p>");
                sb.append(
                        "<p style=\"margin-left: 1.5em; margin-top: 0.25em; margin-bottom: 0.25em;\"><strong>Start Time:</strong> ")
                        .append(escapeHtml(startTime)).append("</p>");
                sb.append(
                        "<p style=\"margin-left: 1.5em; margin-top: 0.25em; margin-bottom: 0.25em;\"><strong>End Time:</strong> ")
                        .append(escapeHtml(endTime)).append("</p>");
            }
        }

        // Full mail for both first run and rerun; only difference is Run Result vs
        // Re-Run #1, #2, ... in the run section above
        ...
        if (failedTestCases != null && failedTestCases.size() > 0) {
            Map<String, List<JsonObject>> testsBySuite = new LinkedHashMap<>();
            for (JsonElement el : failedTestCases) {
                JsonObject tc = el.getAsJsonObject();
                String suiteName = getTestSuiteName(tc);
                testsBySuite.computeIfAbsent(suiteName, k -> new ArrayList<>()).add(tc);
            }

            for (Map.Entry<String, List<JsonObject>> entry : testsBySuite.entrySet()) {
                String suiteName = entry.getKey();
                List<JsonObject> suiteTests = entry.getValue();

                sb.append("<p><strong>Test Suite: ").append(escapeHtml(suiteName)).append("</strong></p>");
                sb.append(
                        "<table border=\"1\" cellpadding=\"8\" cellspacing=\"0\" style=\"border-collapse: collapse; width: 100%; border: 1px solid #000;\">");
                sb.append("<thead><tr style=\"background: #f5f5f5; font-weight: bold;\">");
                sb.append("<th style=\"border: 1px solid #000; text-align: left;\">Test Name</th>");
                sb.append("<th style=\"border: 1px solid #000; text-align: left;\">Environment</th>");
                sb.append("<th style=\"border: 1px solid #000; text-align: left;\">Message</th>");
                sb.append("<th style=\"border: 1px solid #000; text-align: left;\">Link</th>");
                sb.append("</tr></thead><tbody>");
                for (JsonObject tc : suiteTests) {
                    String name = getTestCaseName(tc);
                    String env = getEnvironmentName(tc);
                    String message = tc.has("message") && !tc.get("message").isJsonNull()
                            ? tc.get("message").getAsString()
                            : "";
                    Long tcResultId = tc.has("id") && !tc.get("id").isJsonNull() ? tc.get("id").getAsLong() : null;
                    String detailLink = tcResultId != null
                            ? TESTSIGMA_APP_BASE_URL + "/ui/td/test_case_results/" + tcResultId
                            : liveRunUrl;
                    sb.append("<tr>");
                    ...
                    sb.append("<td style=\"border: 1px solid #000; color: #cc0000;\">").append(escapeHtml(message))
                            .append("</td>");
                    sb.append("<td style=\"border: 1px solid #000;\"><a href=\"").append(escapeHtml(detailLink))
                            .append("\">View Report</a></td>");
                    sb.append("</tr>");
                }
        ...
        if (isFirstRun) {
            sb.append("<p>To view Execution with RunId:").append(runId).append(" in live, <a href=\"")
                    .append(escapeHtml(liveRunUrl)).append("\">click here</a>.</p>");
        } else {
            sb.append("<p>To view Execution with RunId:").append(runIdForViewLink).append(" in live, <a href=\"")
                    .append(escapeHtml(liveRunUrl)).append("\">click here</a>.</p>");
        }
        sb.append("<p>Please click this <a href=\"").append(escapeHtml(xlsReportUrl))
                .append("\">url</a> to generate xls execution report.</p>");
        sb.append(
                "<p>If you face any issues, feel free to contact us at <a href=\"mailto:support@testsigma.com\">support@testsigma.com</a> or use the Instant Chat support available at Testsigma Portal.</p>");
    ...
    private String getTestCaseName(JsonObject tc) {
        if (tc.has("testCaseName") && !tc.get("testCaseName").isJsonNull())
            return tc.get("testCaseName").getAsString();
        if (tc.has("testCaseDetails") && tc.get("testCaseDetails").isJsonObject()) {
            JsonObject d = tc.getAsJsonObject("testCaseDetails");
            if (d.has("name"))
                return d.get("name").getAsString();
        }
    ...
    /**
     * Environment display name (e.g. "Windows Chrome (Real Device)"). Prefers
     * executionEnvironment then environmentSettings.
     */
    private String getEnvironmentName(JsonObject tc) {
        if (tc.has("environmentResult") && tc.get("environmentResult").isJsonObject()) {
            ...
            if (er.has("executionEnvironment") && er.get("executionEnvironment").isJsonObject()) {
                ...
                if (ee.has("name") && !ee.get("name").isJsonNull())
                    return ee.get("name").getAsString();
                if (ee.has("title") && !ee.get("title").isJsonNull())
                    return ee.get("title").getAsString();
            }
    ...
    private String getTestSuiteName(JsonObject tc) {
        if (tc.has("testSuiteName") && !tc.get("testSuiteName").isJsonNull())
            return tc.get("testSuiteName").getAsString();
        if (tc.has("testSuite") && tc.get("testSuite").isJsonObject()) {
            JsonObject s = tc.getAsJsonObject("testSuite");
            if (s.has("name"))
                return s.get("name").getAsString();
        }
        return "N/A";
    }
    ...
    /**
     * Parses TO EMAIL test data: comma-separated list of addresses, trimmed and
     * empty ones removed.
     */
    ...
    private void sendEmail(String host, String port, String user, String password, String from, String to,
