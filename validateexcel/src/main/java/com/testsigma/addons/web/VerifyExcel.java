...
@Data
@Action(actionText = "Verify the value in the Excel filepath with Cell value rowNo and columnNo",
...
public class VerifyExcel extends WebAction {
	...
	public com.testsigma.sdk.Result execute(){
		...
		try {
			...
			String excelFilePath = testData2.getValue().toString();
...
//			FileInputStream inputStream = new FileInputStream(new File(excelFilePath));
			...
			int introw = Integer.parseInt(testData3.getValue().toString());
			int intcolumn = Integer.parseInt(testData4.getValue().toString());

			XSSFRow row=sheet.getRow(introw);

			XSSFCell cell=row.getCell(intcolumn);

			logger.info("got cell value : ");
			...
			if (cell != null) {
				...
				logger.info("verifying cell value : " + CellValue);
