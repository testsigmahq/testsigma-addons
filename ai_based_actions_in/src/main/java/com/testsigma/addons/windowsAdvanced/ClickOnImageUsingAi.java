...
public class ClickOnImageUsingAi extends WindowsAdvancedAction {
    ...
    public Result execute() {
        ...
        try {
            ...
            logger.info(String.format("Clicking via Robot at logical (%d,%d)  confidence=%d", logicalCX, logicalCY, confidence));
            robot.mouseMove(logicalCX, logicalCY);
            robot.mousePress(InputEvent.BUTTON1_DOWN_MASK);
            robot.mouseRelease(InputEvent.BUTTON1_DOWN_MASK);

            setSuccessMessage(String.format(
                    "Successfully clicked '%s' at logical (%d,%d) | capture bbox (%d,%d)-(%d,%d) | confidence=%d | %s",
