...
@Data
@Action(actionText = "Ai: Verify if base pdf base-pdf-file-path and actual pdf actual-pdf-file-path are similar using prompt additional-prompt",
        description = "Renders each page of both PDFs to images and uses AI to compare them page by page. " +
                "Use additional-prompt to specify regions to ignore or aspects to focus on (e.g. 'ignore the footer', " +
                "'ignore timestamps'). Fails if AI detects meaningful differences on any page.",
        displayName = "Ai: Verify if two PDFs are similar by ignoring differences specified in prompt",
        applicationType = ApplicationType.WINDOWS_ADVANCED,
        useCustomScreenshot = true)
public class VerifyIfPdfsAreSimilar extends WindowsAdvancedAction {
    ...
    public Result execute() {
        logger.info("=== VerifyIfPdfsAreSimilar: Starting ===");
        ...
        try {
            ...
            if (basePages != actualPages) {
                setErrorMessage(String.format(
                        "Page count mismatch — base PDF has %d page(s), actual PDF has %d page(s)",
            ...
            for (int i = 0; i < basePages; i++) {
                ...
                BufferedImage baseImg   = baseRenderer.renderImageWithDPI(i, RENDER_DPI);
                BufferedImage actualImg = actualRenderer.renderImageWithDPI(i, RENDER_DPI);
                ...
                try {
                    ...
                    if (node == null) {
                        setErrorMessage("Failed to get AI comparison response for page " + pageNum + " (contact support)");
                    ...
                    if (!match) {
                        ...
                        finalScreenshot = ScreenshotUtils.saveScreenshotToFile(
                                buildSideBySide(baseImg, actualImg, regions, aiWidth, aiHeight, pageNum), "ai_pdf_diff_p" + pageNum);
                        ScreenshotUtils.uploadScreenshotToS3(testStepResult, finalScreenshot, logger);
                        return Result.FAILED;
                    }
                } finally {
            ...
            // All pages passed — upload first page of actual as the step screenshot
            BufferedImage firstActual = actualRenderer.renderImageWithDPI(0, RENDER_DPI);
            finalScreenshot = ScreenshotUtils.saveScreenshotToFile(firstActual, "ai_pdf_passed");
            ScreenshotUtils.uploadScreenshotToS3(testStepResult, finalScreenshot, logger);

            setSuccessMessage(String.format(
                    "AI verified both PDFs are similar across all %d page(s)", basePages));
