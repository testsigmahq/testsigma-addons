...
public class AiActionUtils {
    ...
    public static final String AI_MODEL_ANTHROPIC = "claude-opus-4-7";
    ...
    public static BufferedImage drawHighlight(BufferedImage original,
        ...
        final int GAP      = 20;
        final int SHAFT    = 35;
        final int HEAD_LEN = 10;
        final int HEAD_W   =  7;

        int tipX  = Math.max(HEAD_LEN, cx - GAP);
        int tipY  = cy;
        int tailX = Math.max(0, tipX - HEAD_LEN - SHAFT);
    ...
    public static BufferedImage drawFinalAnnotation(BufferedImage original,
        ...
        int elemH = y2 - y1;
        int INSET = Math.max(2, elemH / 12);
        int ARC   = Math.max(6, elemH / 3);
