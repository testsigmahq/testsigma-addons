...
@Data
@Action(actionText = "SnowflakeDB: Validate Query query1 and compare with the Query query2 from the Connection db-connection-url, private key private-key (Starts with BEGIN PRIVATE KEY)",
        description = "This action validate queries on the database connections",
        applicationType = ApplicationType.WEB)
public class SqlValidateQueriesonTablesUsingPrivateKey extends WebAction {
    ...
    public Result execute() throws NoSuchElementException {
        ...
        String dbConnectionUrl1 = databaseUrl.getValue().toString() + "&private_key_file=" + P8FileCreator(privateKey1);

        try{
            Statement stmt1 = SnowflakeDBConnection.getConnection(dbConnectionUrl1).getConnection().createStatement();
            Statement stmt2 = SnowflakeDBConnection.getConnection(dbConnectionUrl1).getConnection().createStatement();
