...
@Data
@Action(actionText = "Verify if testdata is satisfied (Eg: 5 > 3 OR 4 > 5 AND Testsigma CONTAINS sigma; X CONTAINS (A OR B) or X CONTAINS (A AND B))", description = "Evaluates arithmetic (+, -, *, /), logical (AND, OR), comparison (>, <, >=, <=, ==, !=), and string conditions (CONTAINS, NOT CONTAINS) dynamically within IF steps. CONTAINS (A OR B) expands to CONTAINS A OR CONTAINS B; CONTAINS (A AND B) expands to CONTAINS A AND CONTAINS B.", applicationType = ApplicationType.ANDROID, actionType = StepActionType.IF_CONDITION, useCustomScreenshot = false)
public class IfConditionEvaluatorAction extends AndroidAction {
