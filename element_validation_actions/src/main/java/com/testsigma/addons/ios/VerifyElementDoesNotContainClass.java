...
public class VerifyElementDoesNotContainClass extends IOSAction {
    ...
    protected com.testsigma.sdk.Result execute() {
        ...
        try {
            ...
            WebElement webElement = iosDriver.findElement(this.element.getBy());

            if ("true".equalsIgnoreCase(boolValue.getValue().toString().trim())) {
                logger.info("Scrolling element into view");
                ((JavascriptExecutor) iosDriver).executeScript("arguments[0].scrollIntoView(true);", webElement);
            }
