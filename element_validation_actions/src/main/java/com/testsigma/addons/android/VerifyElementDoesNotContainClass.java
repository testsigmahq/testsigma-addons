...
public class VerifyElementDoesNotContainClass extends AndroidAction {
    ...
    protected Result execute() {
        ...
        try {
            ...
            WebElement webElement = androidDriver.findElement(this.element.getBy());

            if ("true".equalsIgnoreCase(boolValue.getValue().toString().trim())) {
                logger.info("Scrolling element into view");
                ((JavascriptExecutor) androidDriver).executeScript("arguments[0].scrollIntoView(true);", webElement);
            }
