...
@Data
@Action(actionText = "Verify if image image-url is not present in current-page with search threshold threshold-value (Ex: 0.9 , means 90% match) and scale as scale-factor",
        description = "Verify if the given image with threshold and scale is not present in the current page",
        applicationType = ApplicationType.ANDROID,
        useCustomScreenshot = true)
public class VerifyImageNotPresentOnScreenWithThresholdAndScale extends AndroidAction {
    ...
    private com.testsigma.sdk.TestData scaleFactor;
    ...
    protected Result execute() throws NoSuchElementException {
        ...
        try {
            ...
            FindImageResponse response = ocr.findImage(imageURL_,Float.valueOf(thresholdValue.getValue().toString()),Integer.parseInt(scaleFactor.getValue().toString()));
