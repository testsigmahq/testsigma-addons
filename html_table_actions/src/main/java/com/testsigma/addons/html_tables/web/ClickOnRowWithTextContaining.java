...
@Data
@Action(actionText = "Click in the row with text containing test-data of table element",
        description = "Performs click action on the row that contains part of a string",
        applicationType = ApplicationType.WEB)
public class ClickOnRowWithTextContaining extends WebAction {
        ...
        try {
            ...
            for (WebElement row : allrows) {
                ...
                for (WebElement cell : Cells) {
                    if (cell.getText().contains(text.getValue().toString())) {
                        if (!row.isEnabled()) {
