...
@Action(actionText = "store the entire row row-number from sheet sheet-name-or-index in excel/csvfile file-path in format into the runtime variable variable-name",
        description = "Converts a specified row from Excel (XLS/XLSX) or CSV file to JSON format using headers as keys. Supports sheet selection by name or index (0-based)",
        applicationType = ApplicationType.ANDROID,
        useCustomScreenshot = false)
public class ExcelRowToJsonWithSheetAndroidAction extends AndroidAction {
    ...
    public Result execute() throws NoSuchElementException {
        ...
        logger.debug("Row number: " + rowNumber.getValue() + ", File path: " + filePath.getValue() + ", Sheet: " + sheetNameOrIndex.getValue());
        ...
        try {
            ...
            String jsonResult = excelFileUtils.convertRowToJson(filePathStr, rowNum, sheetName, sheetIndex);
            ...
            if (format.getValue().toString().equals("JSON string")) {
                ...
                setSuccessMessage("Successfully converted row " + rowNum + " from sheet '" + (sheetName != null ? sheetName : "index " + sheetIndex) + "' to JSON string and stored in variable: " + varName);
                ...
                setSuccessMessage("Successfully converted row " + rowNum + " from sheet '" + (sheetName != null ? sheetName : "index " + sheetIndex) + "' to JSON file and stored in variable: " + varName);
            }
