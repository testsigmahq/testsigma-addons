...
@Data
@Action(actionText = "Verify if base pdf base-pdf-path and actual pdf actual-pdf-path are similar page by page using prompt additional-prompt",
        description = "Iterates over every page of both PDFs one page per loop iteration. Renders each page to " +
                "an image and uses AI to compare them. " +
                "Exits the loop (FAILED) when all pages have been compared or when a mismatch is detected. " +
                "Fails immediately if the two PDFs have a different number of pages.",
        displayName = "Ai: Verify PDF similarity page by page (while loop)",
        applicationType = com.testsigma.sdk.ApplicationType.WEB,
        useCustomScreenshot = true,
        actionType = StepActionType.WHILE_LOOP)
public class VerifyPdfSimilarityInWhileLoop extends WebAction {
    ...
    private static final String INDEX_KEY = "VerifyPdfSimilarityInWhileLoop_TSIndex";
    ...
    public Result execute() {
        logger.info("=== VerifyPdfSimilarityInWhileLoop: Starting ===");
        ...
        try {
            ...
            if (basePages != actualPages) {
                setErrorMessage(String.format(
                        "Page count mismatch — base PDF has %d page(s), actual PDF has %d page(s). Cannot compare.",
                        basePages, actualPages));
                return Result.FAILED;
            }

            // Resolve current page index persisted from the previous iteration
            Object storedIndex = null;
            try {
                storedIndex = currentIterationIndexProvider.getRuntimeData(INDEX_KEY);
            } catch (Exception ex) {
                logger.info("Failed to read stored page index: " + ex.getMessage());
            }

            int pageIndex = 0;
            if (storedIndex != null) {
                String sv = storedIndex.toString().trim();
                if (!sv.isEmpty()) {
                    try {
                        pageIndex = Integer.parseInt(sv);
                    } catch (NumberFormatException nfe) {
                        logger.info("Non-numeric stored index '" + sv + "', defaulting to 0");
                    }
                }
            }
            logger.info("Current page index (0-based): " + pageIndex);

            // Exit condition: all pages have been processed
            if (pageIndex >= basePages) {
                logger.info("All " + basePages + " page(s) compared successfully. Exiting loop.");
                currentIterationIndex = new com.testsigma.sdk.RunTimeData();
                currentIterationIndex.setKey(INDEX_KEY);
                currentIterationIndex.setValue("");
                setErrorMessage("All " + basePages + " page(s) have been compared. Loop complete.");
                return Result.FAILED;
            }

            int pageNum = pageIndex + 1;
            logger.info("Comparing page " + pageNum + " of " + basePages + "...");

            PDFRenderer baseRenderer   = new PDFRenderer(baseDoc);
            PDFRenderer actualRenderer = new PDFRenderer(actualDoc);

            BufferedImage baseImg   = baseRenderer.renderImageWithDPI(pageIndex, RENDER_DPI);
            BufferedImage actualImg = actualRenderer.renderImageWithDPI(pageIndex, RENDER_DPI);

            File baseJpeg   = AiActionUtils.captureAsJpeg(baseImg,   "ai_pdf_base_p"   + pageNum, logger);
            File actualJpeg = AiActionUtils.captureAsJpeg(actualImg, "ai_pdf_actual_p" + pageNum, logger);
            ...
            try {
                ...
                if (node == null) {
                    setErrorMessage("Failed to parse AI response for page " + pageNum + " (contact support)");
                    finalScreenshot = ScreenshotUtils.saveScreenshotToFile(actualImg, "ai_pdf_error_p" + pageNum);
                ...
                logger.info(String.format("Page %d — match=%b | confidence=%d | desc='%s'",
                        pageNum, match, confidence, description));
                ...
                if (!match) {
                    ...
                    setErrorMessage(String.format(
                            ...
                            pageNum, confidence, description, diffs));
                    ...
                    finalScreenshot = ScreenshotUtils.saveScreenshotToFile(
                            buildSideBySide(baseImg, actualImg, regions, aiWidth, aiHeight, pageNum),
                            "ai_pdf_diff_p" + pageNum);
                    ScreenshotUtils.uploadScreenshotToS3(testStepResult, finalScreenshot, logger);
                    return Result.FAILED;
                }

                finalScreenshot = ScreenshotUtils.saveScreenshotToFile(
                        buildSideBySide(baseImg, actualImg, null, 0, 0, pageNum), "ai_pdf_ok_p" + pageNum);
                ScreenshotUtils.uploadScreenshotToS3(testStepResult, finalScreenshot, logger);
            ...
            // Advance the internal index for the next iteration
            currentIterationIndex = new com.testsigma.sdk.RunTimeData();
            currentIterationIndex.setKey(INDEX_KEY);
            currentIterationIndex.setValue(String.valueOf(pageIndex + 1));
            logger.info("Stored next index " + INDEX_KEY + " = " + (pageIndex + 1));

            setSuccessMessage(String.format(
                    "Page %d of %d: AI verified both PDFs are similar.", pageNum, basePages));
