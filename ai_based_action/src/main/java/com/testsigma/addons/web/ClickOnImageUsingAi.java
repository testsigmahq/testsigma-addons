...
@Data
@Action(actionText = "TESTING Ai: Click on Image/text matching prompt prompt-describing-image",
...
public class ClickOnImageUsingAi extends WebAction {
    ...
    // Coordinates are NOT injected: the AI processes images at an internal resolution that differs
    // from the CSS viewport, so injected CSS pixel values would point to the wrong location in
    // the AI's coordinate space and mislead its reasoning. The green dot + arrow overlay is
    // visually distinctive enough for the AI to locate without pixel hints.
    ...
    public Result execute() {
        ...
        try {
            ...
            // Use the AI's reported visual center (cx/cy) when present; fall back to bbox midpoint.
            int rawCx1 = node1.path("cx").asInt(0);
            int rawCy1 = node1.path("cy").asInt(0);
            int clickX = rawCx1 > 0 ? (int) Math.round(rawCx1 * sx1) : (cX1 + cX2) / 2;
            int clickY = rawCy1 > 0 ? (int) Math.round(rawCy1 * sy1) : (cY1 + cY2) / 2;
            // Clamp to bbox so the AI's center estimate never lands outside the element.
            clickX = Math.max(cX1, Math.min(cX2, clickX));
            clickY = Math.max(cY1, Math.min(cY2, clickY));
            ...
            if (node2 != null) {
                ...
                if (rawX > 0 && rawY > 0 && imgW2 > 0 && imgH2 > 0) {
                    ...
                    if (accurate) {
                        ...
                        // Iteration 2's coordinate nudge is unreliable when it already says accurate.
                        clickX = (cX1 + cX2) / 2;
                        clickY = (cY1 + cY2) / 2;
                    } else {
            ...
            // Upload the final annotated image: bbox rectangle + crosshair + green dot at click center.
            // drawFinalAnnotation (not drawHighlight) is used here so the bounding box makes the
            // center position visually unambiguous to the human reviewer.
            BufferedImage finalAnnotated = AiActionUtils.drawFinalAnnotation(
                    analysisImage, cX1, cY1, cX2, cY2, clickX, clickY);
            ...
            setSuccessMessage(String.format(
                    ...
                    query, clickX, clickY, cX1, cY1, cX2, cY2, conf1, finalDesc));
