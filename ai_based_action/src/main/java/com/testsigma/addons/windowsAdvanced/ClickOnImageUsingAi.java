...
@Action(actionText = "Ai: Click on Image/text matching prompt prompt-describing-image",
        description = "Locate and click a UI element on the desktop using two AI passes: " +
                "Pass 1 locates the element; Pass 2 verifies the green-dot click location and " +
                "corrects it if needed. Uses Robot for HiDPI-aware clicking. " +
                "Fails if the element is not found.",
        displayName = "Ai: Click on Image/text matching prompt",
        applicationType = ApplicationType.WINDOWS_ADVANCED,
        useCustomScreenshot = true)
public class ClickOnImageUsingAi extends WindowsAdvancedAction {
    ...
    public Result execute() {
        logger.info("=== ClickOnImageUsingAi (WindowsAdvanced): Starting ===");
        ...
        try {
            ...
            // Stage A: AI image coords → capture pixel (physical) coords
            ...
            // All values here are in physical pixel space.
            int capCX, capCY;
            if (aiCx > 0 && aiCy > 0) {
            ...
            // Stage B: physical pixels → logical screen coords for Robot.mouseMove
            int logicalCX = (int) Math.round(capCX / displayScaleX);
            int logicalCY = (int) Math.round(capCY / displayScaleY);

            logger.info(String.format(
                    "[Iteration 1] Physical bbox: (%d,%d)-(%d,%d)  physical: (%d,%d)  logical: (%d,%d)",
                    cap[0], cap[1], cap[2], cap[3], capCX, capCY, logicalCX, logicalCY));

            BufferedImage annotated1 = AiActionUtils.drawHighlight(
                    desktopCapture, cap[0], cap[1], cap[2], cap[3], capCX, capCY, Color.MAGENTA);
            annotatedJpegFile = AiActionUtils.captureAsJpeg(annotated1, "ai_desktop_annotated", logger);

            // ──────────────────────────────────────────────────────────────────────
            // ITERATION 2 — Verify the GREEN DOT in the annotated screenshot
            // ──────────────────────────────────────────────────────────────────────
            logger.info(String.format(
                    "[Iteration 2] Verifying green-dot at physical (%d,%d) / logical (%d,%d)...",
                    capCX, capCY, logicalCX, logicalCY));
            ...
            if (node2 != null) {
                ...
                if (rawX > 0 && rawY > 0 && imgW2 > 0 && imgH2 > 0) {
                    ...
                    // Convert updated physical center back to logical for Robot
                    logicalCX = (int) Math.round(capCX / displayScaleX);
                    logicalCY = (int) Math.round(capCY / displayScaleY);
                    ...
                    logger.info(String.format(
                            "[Iteration 2] accurate: %b | corrected: %b | " +
                                    "physical was (%d,%d) → (%d,%d) | logical → (%d,%d) | " +
                                    "delta: dx=%d dy=%d | reason: '%s'",
                            accurate, corrected,
                            prevCapX, prevCapY, capCX, capCY, logicalCX, logicalCY,
                            dx, dy, reason));
                } else {
            ...
            BufferedImage finalAnnotated = AiActionUtils.drawFinalAnnotation(
                    desktopCapture, cap[0], cap[1], cap[2], cap[3], capCX, capCY);
            ...
            setSuccessMessage(String.format(
                    "Successfully clicked '%s' at logical (%d,%d) | physical (%d,%d) | " +
                            "physical bbox (%d,%d)-(%d,%d) | confidence=%d | %s",
                    query, logicalCX, logicalCY, capCX, capCY,
